...
(this does not show proxy options, which are covered in the next section):
  ...
  CASClient::Frameworks::Rails::Filter.configure(
    ...
    :extra_attributes_filter => [:username, :email],
    :logger => cas_logger,
...
<tt>:extra_attributes_session_key</tt> value in the Rails session hash by default. When <tt>:extra_attributes_filter</tt> is
defined then only the whitelisted attributes are retained in the session. Given the above configuration, you would find this info
under <tt>session[:cas_extra_attributes]</tt> containing only the <tt>:username</tt> and <tt>:email</tt> attributes.
